...

  {
    if (argc < 4) {
      {
        fprintf((FILE * __restrict) stdout,
                (char const *__restrict) "incorrect usage\n");
      }
      return;
    }
    { prio = 3; }
...

    {
      while (1) {
        ...
        case_4 : /* CIL Label */
        {
          fscanf((FILE * __restrict) stdin, (char const *__restrict) "%f",
                 &ratio);
        ...
        {
          status = fscanf((FILE * __restrict) stdin,
                          (char const *__restrict) "%d", &command);
        }
      }
