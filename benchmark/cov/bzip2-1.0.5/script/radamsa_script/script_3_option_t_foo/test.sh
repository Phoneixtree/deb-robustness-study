  ...
  while((${cnt} < 500))
  do
  { timeout -k 0.5 0.5 ${origin} -t ${rad_files4bzip2}/foo_fuzzed/foo_${cnt} > foo_${cnt}.bz2; } >/dev/null
  rm foo_${cnt}.bz2
  echo $? >> result/log_origin 

  { timeout -k 0.5 0.5 ${reduced} -t ${rad_files4bzip2}/foo_fuzzed/foo_${cnt} > foo_${cnt}.bz2; } >/dev/null
