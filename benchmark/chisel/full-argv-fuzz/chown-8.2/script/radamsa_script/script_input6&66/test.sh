  ...
  ln -s file1 symfile1
  ln -s file2 symfile2
  { timeout -k 0.5 0.5 ${reduced} < ${rad_files4chown}/input6_fuzzed/input6_${cnt}; } >/dev/null
  ...
  ln -s file1 symfile1
  ln -s file2 symfile2
  { timeout -k 0.5 0.5 ${reduced} < ${rad_files4chown}/input66_fuzzed/input66_${cnt}; } >/dev/null
  echo $? >> result/log_reduced
  rm -rf file1 file2 symfile1 symfile2
