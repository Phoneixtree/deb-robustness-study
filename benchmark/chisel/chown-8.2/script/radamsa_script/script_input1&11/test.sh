  ...
  touch file1
  touch file2 
  { timeout -k 0.5 0.5 ${reduced} < ${rad_files4chown}/input1_fuzzed/input1_${cnt}; } >/dev/null
  echo $? >> result/log_reduced
  rm -rf file1 file2
  
  touch file1
  touch file2 
  { timeout -k 0.5 0.5 ${reduced} < ${rad_files4chown}/input11_fuzzed/input11_${cnt}; } >/dev/null
  echo $? >> result/log_reduced
  rm -rf file1 file2
