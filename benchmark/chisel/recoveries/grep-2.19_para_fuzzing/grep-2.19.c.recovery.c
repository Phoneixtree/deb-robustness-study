                                ...
                                struct quoting_options const *options)
{
  ...
  {
    {
      tmp = __errno_location(); // No.5, Level 2
      e = *tmp;
      n0 = (unsigned int)n;
      sv = slotvec;
    }
...
char const *quote(char const *arg)  // No.102, Level 4 (related to argmatch_valid)
{
  char const *tmp;
...
{ // No.99, Level 4(related to hash_insert)
  void *data;
  struct hash_entry *bucket;
  Hash_tuning const *tuning;
  float candidate;
...
static _Bool enter_dir(FTS *fts, FTSENT *ent) // No.101, Level 4(related to fts_read)
{
...
static int restore_initial_cwd(FTS *sp) // Nｏ.102, Level 4 (related to fts_read)
{
  int fail;
  int tmp;
  int tmp___0;
...
{ // No.100, Level 4:(related to fts_read)
  int open_flags;
  int tmp;
  int tmp___0;
  int fd;
...
FTSENT *(__attribute__((__warn_unused_result__)) fts_read)(FTS *sp)
{
...

  {
    ...
    if (p->fts_level == -1L)
    {
      {
        free((void *)p);
        tmp___13 = __errno_location(); // No.14 , Level 3:
        *tmp___13 = 0;
        tmp___14 = (struct _ftsent *)((void *)0); // END
      }
      return (tmp___14);
    }
    { // No.15 , Level 4:
...
extern pcre *pcre_compile(char const *, int, char const **, int *, unsigned char const *);extern int pcre_exec(pcre const *, pcre_extra const *, char const *, int,
                     int, int, int *, int);
                ...
                char const *start_ptr)
{
...

  {
    ...
    while (1) // NO.38, Level 4(related to while_break)
    {
    while_continue: /* CIL Label */;
...
void dfasyntax(reg_syntax_t bits, int fold, unsigned char eol)
{
  ...
  {                                    // No.53, Level 4(related to syntax_bits_set)
    syntax_bits_set = (reg_syntax_t)1; // END
...
void dfaparse(char const *s, size_t len, struct dfa *d)
{
  ...
  if (!syntax_bits_set) // No.52, Level 1:
  {
    {
...
static int skipped_file(char const *name, int command_line, int is_dir) // No.93, Level 4(related to excluded_patterns, No.92)
{
  _Bool tmp;
  int tmp___0;
  _Bool tmp___1;
...
static int reset(int fd, struct stat const *st)
{
  ...
  char *tmp___1;
  {
    ...
    *(bufbeg + -1) = (char)eolbyte;
    bufdesc = fd;
    if ((st->st_mode & 61440U) == 32768U) // No.60, Level 1
    {
      if (fd != 0)
      {
        bufoffset = (off_t)0;
...
static void prtext(char const *beg, char const *lim)
{
...

  {
    ...
    after_last_match = bufoffset - (buflim - (char *)p); // No.82, Level 4 (related to after_last_match, No.81) // END
    outleft -= n;
  }
}
...
static intmax_t grep(int fd, struct stat const *st)
{
...

  {
    ...
    {
      after_last_match = (off_t)0; // No.83, Level 4(related to after_last_match, No.81) // END
    ...
    // NO.56, Level 4(related to file_is_binary)
    ...
    finish_grep: // No.65, Level 4: (related to finish_grep)
...
static int grepdirent(FTS *fts, FTSENT *ent, int command_line)
{
  ...
  { // No.36 , Level 4: related to fts_set()
    ...
    if ((int)ent->fts_info == 2) // No.67, Level 4(related to case_2, No.66)
    {
      goto case_2;
    }                            // END
    if ((int)ent->fts_info == 4) // No.69, Level 4(related to case_4, No.68)
    {
      goto case_4;
    }
  ...
  case_1: // No.37 , Level 4: related to fts_set()
  ...
  // No.86, Level 1:
  {
    ...
    p = matchers;
    {
      while (1)
      {
        ...
        if (tmp___1 == 0)
        {
          matcher = (char const *)(p->name); // No.87, Level 4 (related to matcher, No.86) // END
...
static size_t prepend_args(char const *options, char *buf, char **argv) // No.98, Level 4(related to No.97)
{
  char const *o;
  char *b;
  size_t n;
...
static int prepend_default_options(char const *options, int *pargc, char ***pargv) // No.97, Level 4 (related to No.96)
{
  char *buf;
  size_t tmp;
  char *tmp___0;
...
int main(int argc, char **argv)
{
  ...
  {
    {
      ...
      filename_mask = ~0;
      max_count = 9223372036854775807L; // No.78, Level 4 (related to max_count, No.77) // END
      atexit(&clean_up_stdout);
      tmp = getenv("GREP_OPTIONS");
      prepended = prepend_default_options((char const *)tmp, &argc, &argv); // No.96, Level 4 (related to prepended, No.94) // END
    ...
    }
    {
      ...
      case_72:
        ...
        goto switch_break;
      case_73: // No.57, Level 4(related to binary_files)
        binary_files = (enum __anonenum_binary_files_72)2;
        goto switch_break; // END
      case_97:             // No.58, Level 4(related to binary_files)
      ...
      case_102: // No.90, Level 2
      {
        ...
        keyalloc = (size_t)1; // No.91, Level 3
        {
          while (1)
          {
          while_continue___0: /* CIL Label */;
      ...
      case_108:
        ...
        goto switch_break;
      case_109:
      { // No.79, Level 4 (related to max_count, No.77)
      ...
      case_120:
        match_lines = 1;
      case_128: // No.59, Level 4(related to binary_files)
      {
  ...
  while_break:;
    ...
    { // No.76, Level 4(related to out_stat)
    ...
    if ((unsigned long)compile == (unsigned long)(&Fcompile)) // No.88, Level 4 (related to matcher, No.86)
    {
      {
    ...
    if (max_count == 0L) // No.80, Level 4 (related to max_count, No.77)
    {
      {
        exit(1);
      }
    ...
    if (optind < argc)
    { // No.95, Level 4(related to status, No.94)
