                               ...
                               char const *vallist, size_t valsize, void (*exit_fn)(void))
{
...

  {
    ...
    // No.33 , Level 4: (related to argmatch_valid)
...
size_t Pexecute(char const *buf, size_t size, size_t *match_size, char const *start_ptr)
{
...

  {
    ...
    line_next = buf;
    {
      // NO.38, Level 4(related to while_break)
      while (1)
      {
      while_continue: /* CIL Label */;
...
static intmax_t grep(int fd, struct stat const *st)
{
  ...
  while_break:;
    ...
    // No.65, Level 4: (related to finish_grep)
  finish_grep:
    done_on_match -= not_text;
    out_quiet -= not_text;
...
static int grepdirent(FTS *fts, FTSENT *ent, int command_line)
{
...

  {
    ...
    // No.67, Level 4(related to case_2, No.66)
    if ((int)ent->fts_info == 2)
    {
      goto case_2;
    }
    // END
    // No.69, Level 4(related to case_4, No.68)
    if ((int)ent->fts_info == 4)
    {
      goto case_4;
    }
    ...
    goto switch_default;
    // No.37 , Level 4: related to fts_set()
  case_1:
    if ((unsigned int)directories == 3U)
    {
      out_file |= 2 * !no_filenames;
...
int main(int argc, char **argv)
{
  ...
  while_break:;
    ...
    // No.88, Level 4 (related to matcher, No.86)
    ...
    // No.80, Level 4 (related to max_count, No.77)
    if (max_count == 0L)
    {
      {
        exit(1);
