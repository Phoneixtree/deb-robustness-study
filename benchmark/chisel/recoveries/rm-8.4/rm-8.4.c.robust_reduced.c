...
_Bool euidaccess_stat(struct stat const *st, int mode) { return ((_Bool)1); }
int dup_safer(int fd) {
  int tmp;
...
_Bool can_write_any_file(void) { return (can_write); }
...
static _Bool check_tuning(Hash_table *table___0) { return ((_Bool)1); }
...
static _Bool setup_dir(FTS *fts) { return ((_Bool)1); }
...
__inline static int priv_set_remove_linkdir(void) { return (-1); }
