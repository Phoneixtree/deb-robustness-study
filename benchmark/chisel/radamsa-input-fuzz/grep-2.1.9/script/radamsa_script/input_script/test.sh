  ...
  { timeout -k 0.5 0.5 ${reduced} "ye " ${rad_files4grep}/input_fuzzed/input_${cnt}; } >/dev/null
  echo $? >> result/log_reduced
  { timeout -k 0.5 0.5 ${reduced} "cold" ${rad_files4grep}/input_fuzzed/input_${cnt}; } >/dev/null
  echo $? >> result/log_reduced
  { timeout -k 0.5 0.5 ${reduced} "not exist" ${rad_files4grep}/input_fuzzed/input_${cnt}; } >/dev/null
  echo $? >> result/log_reduced
  { timeout -k 0.5 0.5 ${reduced} -E '\b(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)' ${rad_files4grep}/input_fuzzed/input_${cnt}; } >/dev/null
  echo $? >> result/log_reduced 
