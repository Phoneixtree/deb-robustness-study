  ...
  mkdir -p d2/d2/d2
  touch d2/d2/d2/file
  { timeout -k 0.5 0.5 ${debloat} < ${rad_files4chown}/input3_fuzzed/input3_${cnt}; } >/dev/null
  ...
  mkdir -p d2/d2/d2
  touch d2/d2/d2/file
  { timeout -k 0.5 0.5 ${debloat} < ${rad_files4chown}/input33_fuzzed/input33_${cnt}; } >/dev/null
  echo $? >> result/log_reduced
  ls -al d1/d1/d1/file | cut -d ' ' -f 4 >temp1
  ls -al d2/d2/d2/file | cut -d ' ' -f 4 >temp2
  diff -q temp1 temp2 >&/dev/null || exit 1
