  ...
  mkdir -p d2/d2/d2
  touch d2/d2/d2/file
  { timeout -k 0.5 0.5 ${debloat} < ${rad_files4chown}/input2_fuzzed/input2_${cnt}; } >/dev/null
  ...
  mkdir -p d2/d2/d2
  touch d2/d2/d2/file
  { timeout -k 0.5 0.5 ${debloat} < ${rad_files4chown}/input22_fuzzed/input22_${cnt}; } >/dev/null
  echo $? >> result/log_reduced
  rm -rf d1 d2
