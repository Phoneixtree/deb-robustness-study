  ...
  ln -s file1 symfile1
  ln -s file2 symfile2
  { timeout -k 0.5 0.5 ${debloat} < ${rad_files4chown}/input7_fuzzed/input7_${cnt}; } >/dev/null
  ...
  ln -s file1 symfile1
  ln -s file2 symfile2
  { timeout -k 0.5 0.5 ${debloat} < ${rad_files4chown}/input77_fuzzed/input77_${cnt}; } >/dev/null
  echo $? >> result/log_reduced
  rm -rf file1 file2 symfile1 symfile2
