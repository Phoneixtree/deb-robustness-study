...
static char** afl_init_argv(int* argc) {
  ...
  //tatic char  in_buf[MAX_CMDLINE_LEN];
...
{ 
  //AFL_INIT_ARGV();
  AFL_INIT_SET0("chown-8.2.c.orig.c");
  _Bool preserve_root;
  uid_t uid ;
  gid_t gid ;
  uid_t required_uid ;
...
/*int main(int argc, char **argv) {
  int new_argc = 1;
  char arg_line[100];
  FILE* arg_f = fopen(argv[1], "r");
  fgets(arg_line, 100, arg_f);
  fclose(arg_f);

  char str0[100];
  char str1[100];
  strcpy(str0, arg_line);
  strcpy(str1, arg_line);

  char* ptr = strtok(str0, " ");
  while (ptr != 0) {
    new_argc += 1;
    ptr = strtok(0, " ");
  }
