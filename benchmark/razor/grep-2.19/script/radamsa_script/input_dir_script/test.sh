  ...
  { timeout -k 0.5 0.5 ${debloat} "a" -v -H -r ${rad_files4grep}/input_dir_fuzzed/input_dir_${cnt}; } >/dev/null
  echo $? >> result/log_reduced 
  { timeout -k 0.5 0.5 ${debloat} "1" -h -r ${rad_files4grep}/input_dir_fuzzed/input_dir_${cnt}; } >/dev/null
  echo $? >> result/log_reduced 
  { timeout -k 0.5 0.5 ${debloat} "1" -r ${rad_files4grep}/input_dir_fuzzed/input_dir_${cnt} -l ; } >/dev/null
  echo $? >> result/log_reduced 
  { timeout -k 0.5 0.5 ${debloat} "1" -r ${rad_files4grep}/input_dir_fuzzed/input_dir_${cnt} -L ; } >/dev/null
  echo $? >> result/log_reduced   
  { timeout -k 0.5 0.5 ${debloat} "randomtext" -r ${rad_files4grep}/input_dir_fuzzed/input_dir_${cnt} -c ; } >/dev/null
  echo $? >> result/log_reduced 
  { timeout -k 0.5 0.5 ${debloat} "1" -r ${rad_files4grep}/input_dir_fuzzed/input_dir_${cnt} -q ; } >/dev/null
  echo $? >> result/log_reduced 
  { timeout -k 0.5 0.5 ${debloat} "1" -r ${rad_files4grep}/input_dir_fuzzed/input_dir_${cnt} -s ; } >/dev/null
  echo $? >> result/log_reduced 
